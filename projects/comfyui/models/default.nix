{
  ...
  # TODO: it's a bit messy at the moment with regard to meta data due to the lack of standard terminology and classifications,
  # and while e.g. the civitai website may state that a lora has "Flux.1 D" as base model, the equivalent component of its AIR
  # is ecosystem=flux1. Probably should adopt the AIR way going forward.
  meta = import ./meta.nix;
  inherit (meta.base-models) flux1d flux1s sd15 sdxl svd svdxt clip_vision inpaint controlnet config ipadapter upscaler;
  inherit (meta.model-types) checkpoint lora clip text_encoder vae;
  fetchFromUrl = import <nix/fetchurl.nix>;
in
  installByAir {
    # you can get both AIR and hash directly from the page of a model on civitai
    # template:
    # "loras/example.safetensors" = { air = "urn:air:flux1:lora:civitai:<id>@<version>"; sha256 = "<hash>"; };

    # https://civitai.com/models/685229/digital-25d-art-zerokelvin?modelVersionId=766909
    "loras/digital-25d-art-zerokelvin.safetensors" = {
      air = "urn:air:flux1:lora:civitai:685229@766909";
      sha256 = "F11ADB0A537CC743B0DD0CABACF6425ECC586B7FD2196C7262EAD9242632AEE8";
    };
  }
  // {
